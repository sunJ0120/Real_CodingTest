...
- 시뮬레이션 문제를 풀 때 중요한 것은, 과도한 추상화 및 분리를 하지 않고 있는 그대로 푸는 것이다.
- 그래서 후진 = (dir + 2) % 4;로 정의하고 회전 = (dir + 3) % 4;로 수식화 해서 정의
- 나의 경우는 우테코를 하다보니 메서드 분리가 습관처럼 굳어진 느낌인데, 코테용과 플젝용을 분리하자.
- 아 그리고... 헷갈리지 말길 북쪽이 위라서 -1이 맞음요


메모리 : 14332KB  | 시간 : 104ms